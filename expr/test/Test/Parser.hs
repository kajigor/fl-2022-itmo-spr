...
testParserBracketsSuccess :: (String -> Maybe Expr) -> String -> String -> Assertion
testParserBracketsSuccess parser input expected = do
  let res = parser input
  assertBool "Expected parsing to succeed" (isJust res)
  res @?= parser expected
...
unit_parseInfix = do
  ...
  syntaxError parser "+12"
  syntaxError parser ""
  syntaxError parser " 1 + 2 3  "

  parsingSuccess parser "123" (Num 123)
  parsingSuccess parser " 1 + 23  " (plus (Num 1) (Num 23))
  parsingSuccess parser "12+3" (plus (Num 12) (Num 3))
